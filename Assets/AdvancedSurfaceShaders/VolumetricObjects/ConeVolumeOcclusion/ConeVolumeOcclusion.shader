Shader "Advanced SS/Volumetric/Cone Volume Occlusion"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        {	
            ...
            // CONE INTERSECTION
            //==============================
            inline float IntersectCone(float3 rayOrigin, float3 rayDirection, float coneRadiusOverHeightSqr, float coneHeight, float topCap, float4x4 transformMatrix, inout float t1, inout float t2)
            {
                // Quadratic:
                // ax^2 + bx + c = 0
                ...
                //float k = coneRadiusOverHeight;//coneRadius / coneHeight;
                float ksquared = coneRadiusOverHeightSqr;//k*k;
            ...
            VSOutput vert (appdata_full IN)
            {
                ...
                OUT.vWorldPos.xyz = mul((float4x4)_Object2World, float4(IN.vertex.xyz, 1.0)).xyz;
                OUT.vViewPos = mul((float4x4)UNITY_MATRIX_MV, float4(IN.vertex.xyz, 1.0)).xyz;
                OUT.vLocalCamPos = mul((float4x4)_World2Object, (float4(_WorldSpaceCameraPos, 1.0))).xyz;
            ...
            float4 frag (VSOutput IN) : COLOR
            {
                ...
                float bValidIntersection = IntersectCone(rayOrigin, rayDirection, pow(_ConeData.x / _ConeData.y, 2), _ConeData.y, _ConeData.z, _World2Object, t1, t2);
                ...
                float invDoubleRadius = 1.0f / (_ConeData.x * 2.0f);
                //float noiseValue = 1;
                ...
                for(int s=0; s<sampleCount; s++)
                {
                    ...
                    // Occlusion
                    float depth01 = -samplePosLocal.y * invConeHeight;
                    float invHeightMod = 1.0f / depth01;
                    float2 coneCoords = float2((((samplePosLocal.x + _ConeData.x) * invDoubleRadius) - 0.5f) * invHeightMod, (((samplePosLocal.z + _ConeData.x) * invDoubleRadius) - 0.5f) * invHeightMod);

                    float sampleTexDepth = 1 - DecodeFloatRGBA(tex2D(_OcclusionTex, coneCoords.xy + float2(0.5f,0.5f)));
                    //float occlusionDepth = 1.0 - sign(depth01-sampleTexDepth);
                    float occlusionDepth = step(depth01, sampleTexDepth);
                    //float occlusionIntensity = saturate(1f - _Visibility / (occlusionDepth - samplePosLocal.y));
                    float occlusionIntensity = occlusionDepth;//saturate(occlusionDepth - coneCoords.z);//saturate(samplePosLocal.y - occlusionDepth);
                    ...
                    intensity += (invSampleCount * noiseValue) * occlusionIntensity;// * (1f - heightAttenuation * _Visibility.y));// * angleAttenuation);// * distanceAttenuation;
                }

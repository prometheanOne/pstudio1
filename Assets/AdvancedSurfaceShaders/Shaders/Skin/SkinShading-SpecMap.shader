Shader "Advanced SS/Skin/Skin Shading (Spec Map)" {
	Properties {
        _Color ("Main Color", Color) = (1,1,1,1)
		_MainTex ("Base (RGB)", 2D) = "white" {}
		_BumpMap ("Bumpmap", 2D) = "bump" {}
        _SpecMap ("Specular (RGB), Gloss (A)", 2D) = "white" {}
	...
	SubShader {
		...
		half4 LightingSSS (SurfaceOutputSSS s, half3 lightDir, half3 viewDir, half atten) {
			...
			float3 specular = pow (nh, s.Specular*128.0) * linearLightColour * s.GlossColour;
		...
		void surf (Input IN, inout SurfaceOutputSSS o)
        {
            ...
            half4 specMapCol = tex2D(_SpecMap, IN.uv_MainTex);
            o.GlossColour = specMapCol.rgb;
            o.Specular = specMapCol.a * _Shininess;

    ...
    SubShader {
    	Pass {
            ...
            CGPROGRAM
                ...
                float4 frag(v2f i) : COLOR
                {
                    ...
                    fixed NdotL = max(0,dot(n, i.lightDir));
                    fixed NdotH = max(0,dot(n, h));
                    fixed N2dotH = max(0,dot(n2, h));
                    fixed N2dotV = 1.0 - saturate(dot (normalize(i.viewDir), n2));
                    
                    half rim = pow(N2dotV, 4);
                    
                    half3 spec = pow(NdotH, 40) * masks.r * _LightColor0.rgb;
                    ...
                    fixed4 c;
                    c.rgb = ((albido * _LightColor0.rgb * NdotL)  + spec) * (atten * 2) + ((reflection + rim) * masks.b);
		...
		Pass {
            ...
            CGPROGRAM
                ...
                #pragma fragmentoption ARB_precision_hint_fastest
                #pragma only_renderers d3d9 d3d11 gles
                #pragma target 3.0
                ...
                float4 frag(v2f i) : COLOR
                {
                    ...
                    fixed4 c;
                    c.rgb = ((albido * _LightColor0.rgb * NdotL) + spec) * (atten * 2);
                    c.a = 1;
                    return c;
                }
            ENDCG
